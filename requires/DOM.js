...
// @author                Beastx
//
// @history                0.2 $$ function now use a cssQuery Library
// @history                0.2 Added cssQuery library
// @history                0.2 Added insertAfter method
...
function NewModule(classRef, constructorArgs, events) {
    if (!classRef.prototype.getConfigs) {
        classRef.prototype.getConfigs = function() {
            return {};
        }
    }
    var obj = New(classRef, constructorArgs, events);
        ...
        for (var i = 0; i < rowsArray.length; ++i) {
            tbody.appendChild(
                DOM.createElement('tr', null, [
                    DOM.createElement('th', { style: { width: '100%', borderRight: '1px solid #BBB' } }, [
                        DOM.createElement('label', null, [ rowsArray[i].label ])
                    ]),
                    DOM.createElement('td', { style: { textAlign: 'left', padding: '4px 8px', width: '0%', backgroundColor: '#FAEAC6' } }, [ rowsArray[i].value ])
                ])
            )
        }
        return DOM.createElement('table', {cellSpacing: 0, cellPadding: 0, style: { width: '100%', borderTop: '1px solid #999', marginTop: '10px' }}, [ tbody ])
    }
    return obj;
}
...
DOM.getWindowInnerSize = (function() {
    ...
    function isDocumentElementHeightOff(window) {
        div = document.createElement('div');
        div.style.height = "2500px";
        document.body.insertBefore(div, document.body.firstChild);
        var r = document.documentElement.clientHeight > 2400;
        document.body.removeChild(div);
        return r;
    }
    var bodyIsActingAsRoot = document.documentElement && document.documentElement.clientWidth === 0;
    var node;
    if (typeof document.clientWidth == "number") { // Safari 2 uses document.clientWidth
        node = window.document;
        propPrefix = "client";
    } else if (bodyIsActingAsRoot || isDocumentElementHeightOff(window)) { // Opera < 9.5, or IE in quirks mode.
        node = document.body;
        propPrefix = "client";
    } else if (document.documentElement.clientHeight > 0) { // Modern Webkit, Firefox, IE, might be undefined. 0 in older mozilla.
        node = document.documentElement;
        propPrefix = "client";
    } else if (typeof innerHeight == "number") { // For older versions of Mozilla.
        node = window;
        propPrefix = "inner";
    }
    var widthProperty = propPrefix + 'Width';
    var heightProperty = propPrefix + 'Height';
    return function() {
        return { w: node[widthProperty], h: node[heightProperty] };
    }
})();

DOM.addOrderLinksToTable = function(tableElement, getContentForColumnCallBack, defaultOrderColumnIndex) {
    var thead = tableElement.childNodes[1];
    var tbody = tableElement.childNodes[3];
    ...
    function orderBy(column, direction) {
        function sortFunction(a, b){
            if (direction) {
                var contentForA = getContentForColumnCallBack(ths[column].textContent.replace('⇑', '').replace('⇓', '').trim(), VAR.filter(a.childNodes, DOM.cleanChildNodes)[column]);
                var contentForB = getContentForColumnCallBack(ths[column].textContent.replace('⇑', '').replace('⇓', '').trim(), VAR.filter(b.childNodes, DOM.cleanChildNodes)[column]);
            } else {
                var contentForA = getContentForColumnCallBack(ths[column].textContent.replace('⇑', '').replace('⇓', '').trim(), VAR.filter(b.childNodes, DOM.cleanChildNodes)[column]);
                var contentForB = getContentForColumnCallBack(ths[column].textContent.replace('⇑', '').replace('⇓', '').trim(), VAR.filter(a.childNodes, DOM.cleanChildNodes)[column]);
            }
            if (VAR.isNumber(contentForA) && VAR.isNumber(contentForB)) {
                return (contentForA-contentForB);
            }
            var sortedValues = [contentForA, contentForB].sort();
            if (sortedValues[0] == contentForA) {
                return -1
            } else if (sortedValues[1] == contentForA) {
                return 1;
            } else {
                return 0;
            }
        }
        var orderedTrs = trs.sort(sortFunction);
        DOM.removeAllChildNodes(tbody);
        DOM.appendChildNodes(tbody, orderedTrs);
    }
    
    for (var i = 0; i < ths.length; ++i) {
        var thName = ths[i].firstChild.textContent.trim();
        if (thName != 'Acciones') {
            DOM.removeAllChildNodes(ths[i]);
            (function(i) {
                ths[i].appendChild(DOM.createElement('span', null, [ thName ]));
                ths[i].appendChild(DOM.createElement('a', { 'class': 'unicode', href: '#', onclick: function(event) { DOM.cancelEvent(event); orderBy(i, 0); } }, [ '⇑' ]));
                ths[i].appendChild(DOM.createElement('a', { 'class': 'unicode', href: '#', onclick: function(event) { DOM.cancelEvent(event); orderBy(i, 1); } }, [ '⇓' ]));
            })(i);
        }
    }
    orderBy(defaultOrderColumnIndex, 0);
}
