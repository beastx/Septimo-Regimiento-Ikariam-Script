...
ScriptUpdater = {
    version:"0.1",
    //------------------------------------------- "public" methods --------------------------------------
    check:function(scriptId, currentVersion, callback, isBaseScript) {    
        ScriptUpdater.isBaseScript = isBaseScript;
    ...
    initVars:function(scriptId, currentVersion, callbackFunction, useNotice, forceNotice) {
    ...
    checkRemoteScript:function() {
        Beastx.log('Check: ' + ScriptUpdater.scriptId);
        ...
        if (!ScriptUpdater.isBaseScript) {
            var url = Beastx.Config.options.serverUrl + "/tools/userScripts/Beastx." + ScriptUpdater.scriptId + '.user.js';
        } else {
            var url = Beastx.Config.options.serverUrl + "/tools/userScripts/requires/" + ScriptUpdater.scriptId + '.js';
        }
        GM_xmlhttpRequest ({
            method: "GET",
            url: url,
